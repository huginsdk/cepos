...
namespace Hugin.POS
{
    ...
    static class BackgroundWorker
    {
        ...
        private static int maxZCount =0;
        private static int MaxZCount
        {
            get
            {
                try
                {
                    if (maxZCount == 0)
                    {
                        maxZCount = int.Parse(PosConfiguration.Get("MaxZCount"));
                    }
                }
        ...
        private static void AutoLoadOrders(DocumentFileHelper[] orders)
        {
            ...
            {
                ICashier cashier = Data.Connector.Instance().FindCashierById("0006");
                if (cashier != null)
                {
            ...
            foreach (DocumentFileHelper helper in orders)
            {
                ...
                {                    
                    ...
                    if (docCount > MaxZCount) //when number of document for a z exceeds limit print z
                    {
                ...
                {
                    cr.Log.Error("Exception occur on order printing Receipt Number: {0}  Order File: {1}", helper.FileInfo.Name, docCount);
                    cr.Log.Error(ex);
                }
                /**/
                docCount++;
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        */
                        DisplayAdapter.Cashier.Show("FÝÞ NO", docCount, "TUTAR", cr.Document.TotalAmount, false);
                        cr.Document.BalanceDue = 0;
                        cr.Document.Close();
