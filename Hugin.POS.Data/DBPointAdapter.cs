...
namespace Hugin.POS.Data
{
    ...
    {
        private static OleDbConnection oleDbCnn = null;
        ...
        internal DBPointAdapter(String path)
        {
            try
            {
                oleDbCnn = new OleDbConnection(String.Format("Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}", path ));
        ...
        public bool Online
        {
            get
            {
                ...
                try
                {
                    int count = (int)GetOleDbValue("SELECT Count(CustomerCode) FROM TblDaily");
                    retVal = true;
                }
                catch { }
        ...
        {
            OleDbCommand oleDbCmd = new OleDbCommand(strQuery, oleDbCnn);
            int rowsEffected = 0;
            try
            {
        ...
        private int ExecuteCommand(OleDbCommand sqlDbCmd)
        {
            int rowsEffected = 0;
            try
            {
        ...
        {
            OleDbCommand oleDbCmd = new OleDbCommand(strSelect, oleDbCnn);
        ...
        private void ControlServerForUpdates()
        {
            ...
            while (true)
            {
                ...
                List<String> lines = new List<string>(File.ReadAllLines(LogPath));
