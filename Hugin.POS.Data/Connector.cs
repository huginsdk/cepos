...
namespace Hugin.POS.Data
{
    ...
    public class Connector:IDataConnector
    {
        ...
        public event LogSavedEventHandler LogSaved;
        ...
        private Connector()
        {
            ...
            {
                fxClient = new TransferClientFtp(PosConfiguration.Get("OfficeLogin"));
            }
        ...
        public void LoadSettings()
        {
            try
            {
                ...
                if (fxClient.DirectoryExists(PosConfiguration.ServerDataPath, 3000))
                {
                    ...
                    dataFileList.Add(Settings.CategoryFile, PosConfiguration.ServerDataPath);
        ...
        {
            lastSuccess[DataTypes.Category] = 0;
            lastFail[DataTypes.Category] = 0;

            String path = PosConfiguration.DataPath + Settings.CategoryFile;
            if (!File.Exists(path))
            {
                EZLogger.Log.Error("Kategori dosyasý bulunamadý");
                return categoryList;
            }
        ...
        private void ReadCategoryFile(String path)
        {
            ...
            while ((line = @sr.ReadLine()) != null)
            {
                if (line.Trim().Length == 0 || line[0] == '0')
                    continue;
                if (AddLineFromCategoryFile(line))
                    lastSuccess[DataTypes.Category]++;
                else
                    lastFail[DataTypes.Category]++;

#if WindowsCE
                if (lastSuccess[DataTypes.Product] >= MAX_PRODUCT_COUNT)
                {
                    EZLogger.Log.Info("Maksimum ürün sayýsýna ulaþýldý : {0} ", lastSuccess[DataTypes.Product]);
                    break;
                }
#endif
        ...
        private bool AddLineFromCategoryFile(String line)
        {
            try
            {
                Category c = new Category();

                c.valid = Int32.Parse(line.Substring(0, 1)) == 1;
                c.MainCatNo = Int32.Parse(line.Substring(1,2));
                c.CatNo = Int32.Parse(line.Substring(3,4));
                c.Name = line.Substring(7);
        ...
        public void LoadProducts()
        {
            ...
            if (!File.Exists(path))
            {
                EZLogger.Log.Error("Ürün dosyasý bulunamadý");
                return;
            }
        ...
        public void UpdateProducts()
        {
            ...
            //Update URUN.DAT
            string backupFile = PosConfiguration.DataPath + "BackupProduct.dat";
            try
            {
                
                EZLogger.Log.Info("BACKUP EXIST PRODUCT FILE");
                File.Move(PosConfiguration.DataPath + Settings.ProductFile, backupFile);
                
                EZLogger.Log.Info("PRODUCT TO LINE");
                Product.ProductsToFile(PosConfiguration.DataPath + Settings.ProductFile);

                EZLogger.Log.Info("DELETING TEMPORARY PRODUCT FILE :" + path);
            ...
            {
                if (File.Exists(backupFile))
                    File.Move(backupFile, PosConfiguration.DataPath + Settings.ProductFile);

                EZLogger.Log.Error("Ürün dosyasý yazýlamadý. Hata: " + ex.Message);
                //think can be solvable without exception, and should success and fail counters be reset.
                throw new IOException("URUN DOSYASI GUNCELLENEMEDI");
            }
            if (ProductsUpdated != null)
                ProductsUpdated(null, new EventArgs());
        ...
        public void UpdateCustomers()
        {
            ...
            String path = PosConfiguration.DataPath + "Update" + Settings.CustomerFile;
            if (!File.Exists(path))
            {
                EZLogger.Log.Error("Müþteri güncelleme dosyasý bulunamadý");
                return;
            }

            // Reads customer file, parses lines by Parse(...) function, updates customer fields..
            EZLogger.Log.Info("MÜÞTERÝ DOSYASI GÜNCELLENÝYOR");
            ReadCustomerFile(path);

            //Update CARI.DAT
            string backupFile = PosConfiguration.DataPath + "BackupCari.dat";
            try
            {

                EZLogger.Log.Info("BACKUP EXIST CUSTOMER FILE");
                File.Move(PosConfiguration.DataPath + Settings.CustomerFile, backupFile);

                EZLogger.Log.Info("CUSTOMER TO LINE");
                //Product.ProductsToFile(PosConfiguration.DataPath + Settings.ProductFile);
                Customer.CustomersToFile(PosConfiguration.DataPath + Settings.CustomerFile);
            ...
            catch (Exception ex)
            {
                ...
                EZLogger.Log.Error("Müþteri dosyasý yazýlamadý. Hata: " + ex.Message);
                //think can be solvable without exception, and should success and fail counters be reset.
                throw new IOException("MUSTERI DOSYASI GUNCELLENEMEDI");
            }

            EZLogger.Log.Info("Müþteri güncelleme bilgisi : Baþarýlý {0} Baþarýsýz {1}",
        ...
        private static void ReadProductFile(string path)
        {
            ...
            while ((line = @sr.ReadLine()) != null)
            {
                ...
                if (line.Trim().Length == 0 || line[0] == '0')
                    continue;
                if (Product.Add(line))
                    lastSuccess[DataTypes.Product]++;
                else
                    lastFail[DataTypes.Product]++;
        ...
        private static void ReadCustomerFile(string path)
        {
            StreamReader sr = new StreamReader(path, PosConfiguration.DefaultEncoding);
            String line = "";
        ...
        public List<ICashier> SearchCashiersByInfo(String info)
        {
            return Cashier.SearchCustomers(info);
        }
        ...
        public ICustomer FindCustomerByCode(String customerNumber)
        {
            return Customer.FindByCode(customerNumber);
        }

        public ICustomer FindCustomerByCardNo(String cardNo)
        {
            return Customer.FindByCardNo(cardNo);
        }
        ...
        public ICustomer FindCustomerByTcknVkn(String tcknVkn)
        {
            return Customer.FindByTcknVkn(tcknVkn);
        }
        ...
        public ICustomer SaveCustomer(String line)
        {
            ...
            try
            {
                using (sw = new StreamWriter(path, true, PosConfiguration.DefaultEncoding))
                {
        ...
        public void StartLog(String exeVersion)
        {
            ...
            mainLog.PrintExeVersion(exeVersion);
        }
        ...
        {
            string log = mainLog.OnDocumentClosed(document);
            LogSaved(this, new LogSavedEventArgs(log));
            lastDocumentLog.OnDocumentChanged(null, 0);
        }

        public void OnDocumentVoided(ISalesDocument document, int voidedReason)
        {
            string log = voidedDocumentsLog.OnDocumentVoided(document, voidedReason);
            LogSaved(this, new LogSavedEventArgs(log));
            lastDocumentLog.OnDocumentChanged(null, 0);
        }
        ...
        public string[] GetReturnAmounts(string registerId) { return Document.GetReturnAmounts(registerId); }
