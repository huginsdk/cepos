...
{
    class CategoryPromotion : BasePromotion, IPromotion
    {
        ...
        internal static void Load()
        {
            ...
            CategoryPromotion pro;

            try
            {
                sr = new StreamReader(Settings.DataPath + Settings.CATEGORY_PROMOTION_FILE_NAME, Settings.DefaultEncoding);
            ...
            using (sr)
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    try
                    {
                        ...
                        pro = new CategoryPromotion();
                        pro.id = Int32.Parse(lineArray[0]);
                        lineNo++;
                        // pro.range = LineArray[1] == "H" ? PromotionRange.ALL : PromotionRange.CUSTOMER;
                        switch (lineArray[1])
                        {
                            case "H":
                        ...
                        pro.requiredAmount = pro.LimitType != LimitType.NoLimit ? pro.Discount : 0m;
                        ...
                        {
                            if (Parser.TryLong(lineArray[counter], out groupId))
                                pro.customerGroups.Add(lineArray[counter]);
                            else
                                pro.promoRemark = lineArray[counter];
                        }
                        pro.code = "R" + (lineNo).ToString();
                    ...
                    {
                        lastException = e;
                        Settings.Log("Proreyon.dat okuma hatasi! " + e.Message);
                        continue;
                    }
                }
            }
        ...
        public decimal TotalAmount()
        {
            ...
            if (rows.Length > 0)
                total = Convert.ToDecimal(SoldItem.Instance().Compute("Sum(Amount)", "CategoryId = '" + this.Id + "'"));
            return total;
        }
        ...
        public void ApplyPromotion()
        {
            ...
            switch (base.LimitType)
            {
                ...
                case LimitType.Amount:
                    ratio = (int)(TotalAmount() / base.RequiredAmount);
            ...
            DataRow[] items = SoldItem.Instance().Select(String.Format("CategoryId = '{0}'", base.Id));
            ...
            decimal appliedDisc = (decimal)SoldItem.Instance().Compute("Sum(DiscountAmount)", String.Format("CategoryId = '{0}'", base.Id));
