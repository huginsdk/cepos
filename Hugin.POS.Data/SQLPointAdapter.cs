...
{
    internal class SQLPointAdapter :IPointAdapter
    {
        ...
        public bool Online
        {
            get
            {
                ...
                try
                {
                    int count = (int)GetSqlDbValue("SELECT Count(CustomerCode) FROM TblDaily");
                    retVal = true;
                }
                catch { }
        ...
        public bool HasOfflineRecords
        {
            get
            {
                try
                {
                    ...
                    {
                        return File.ReadAllText(this.LogPath).Length > 0;
                    }
                }
                catch
                {
        ...
        public void AddCard(ICustomer customer, string cardSerial)
        {
            ...
            int count = Convert.ToInt32(GetSqlDbValue(strSelect));
            if (count > 0)
        ...
        public void UpdatePoint(PointObject pointObj)
        {
            ...
            EZLogger.Log.Info(String.Format("Müþteri puaný kayýt ediliyor. M:{0}  P:{1}", pointObj.Customer.Code, pointObj.Customer.Points));

            try
            {
                if (CheckAlreadyAdded(pointObj))
                {
                    EZLogger.Log.Info("Kayýt daha önceden eklenmiþ." + pointObj.ToString());
                    return;
                }
            ...
            SqlCommand command = new SqlCommand(strUpdate, sqlConnection);
            ...
            EZLogger.Log.Info(String.Format("Müþteri puaný kayýt edildi. M:{0}  P:{1}", pointObj.Customer.Code, pointObj.Customer.Points));
            ...
            command = new SqlCommand(strUpdate, sqlConnection);

            command.Parameters.AddWithValue("@points", pointObj.Value);
            command.Parameters.AddWithValue("@customerCode", pointObj.Customer.Code);
            command.Parameters.AddWithValue("@documentDate", pointObj.DocumentDate);
        ...
        public long GetPoint(ICustomer customer)
        {
            ...
            if (count > 0)
            {
                ...
                return Convert.ToInt64(GetSqlDbValue(strSelect));
            }
            else
        ...
        public bool Invalid(string cardSerial)
        {
            ...
            if (count > 0)
            {
                ...
                int valid = Convert.ToInt32(GetSqlDbValue(strSelect));
                return valid == 1;
            }
            else
        ...
        {
            SqlCommand sqlDbCmd = new SqlCommand(strQuery, sqlConnection);
            int rowsEffected = 0;
            try
            {
        ...
        private Object GetSqlDbValue(string strSelect)
        {
            SqlCommand sqlDbCmd;
            lock (sqlDbCmd = new SqlCommand(strSelect, sqlConnection))
            {
                SqlDataReader sqlDbReader = null;
        ...
        {
            File.AppendAllText(LogPath, query + Environment.NewLine);
        }
        ...
        private void ControlServerForUpdates()
        {
            while (true)
            {
                ...
                String strQuery = DequeueLine(LogPath);
                while (strQuery != "")
                {
                    try
                    {
                    ...
                    {
                        EZLogger.Log.Error("Hatalý puan satýrý : {0}", strQuery);
                    }
                    catch (Exception ex)
                    {
                        EZLogger.Log.Error("Puan satýrý kayýt hatasý : {0} \n Error: {1}", strQuery, ex.Message);
                        break;
                    }
                    strQuery = DequeueLine(LogPath);
                }
            }
        }
        ...
        private string DequeueLine(string LogPath)
        {
            List<string> quotelist = new List<string>(File.ReadAllLines(LogPath));
            string firstItem = "";
            if (quotelist.Count > 0)
            {
                firstItem = quotelist[0];
                quotelist.RemoveAt(0);
                File.WriteAllLines(LogPath, quotelist.ToArray());
            }
            return firstItem;
        }
        ...
        private PointObject StringToObject(string strQuery)
        {
            try
            {
                ...
                po.Customer = Customer.FindByCardNo(splitted[1]);
